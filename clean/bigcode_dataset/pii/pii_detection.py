...
try:
    ...
    from clean.bigcode_dataset.pii.utils.emails_ip_addresses_detection import detect_email_addresses
    from clean.bigcode_dataset.pii.utils.keys_detection import detect_keys
...
def scan_pii_batch(examples, key_detector="other"):
    ...
    if list_secrets:
        filter_list_secrets=[elem for elem in list_secrets if (elem and elem!='[]')]
        if filter_list_secrets:
            logger.info(f'list_secrets[:10]={str(list_secrets[:10])[:100]}')
    return {

...
def is_gibberish(matched_str):
    ...
    Detector = detector.create_from_model(os.path.join(project_root,'gibberish_data/big.model'))
...
def is_hash(content, value):
    ...
    try:
        ...
        logger.error("Value not found in content, why this happened?")
        return False
...
def detect_keys(content, suffix=".txt"):
    ...
    if secrets_set:
        for secret_elem in secrets_set:
            if isinstance(secret_elem,list):
                for sub_secret_elem in secret_elem:
                    logger.info(f"secrets_set:{sub_secret_elem.json()}")
    matches = []
    if secrets_set:

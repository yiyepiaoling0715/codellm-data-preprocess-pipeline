...
def get_argparse():
    ...
    parser.add_argument("--dataset_path_after_dedup", type=str)

    parser.add_argument("--ngram_size", type=int, default=10)
    parser.add_argument("--threshold", type=float, default=0.7)
    parser.add_argument("--num_perm", type=int, default=128)
    parser.add_argument("--min_ngram_size", type=int, default=10)
    parser.add_argument("--remove_columns", type=str,default=None)
    parser.add_argument("--save_mode", type=str, choices=['save_to_disk','save_to_json'],default='save_to_disk')
...
class UnionFind:
    ...
    def find(self, x):
        """
            溯源找到 top parent,标识就是 parent[x]==x
        """
    ...
    def union(self, x, y):
        """
            联合,在最顶层进行赋值,每个cluster只保留一个parent
        """
        px = self.find(x)
        py = self.find(y)
...
def run(args):
    ...
    # output=args.output
    dataset_path_after_dedup=args.dataset_path_after_dedup
    ...
    if args.remove_columns:
        remove_columns=args.remove_columns.split(",")
    ...
    assert os.path.isdir(dataset), f"Dataset not found dataset={dataset}"
    ...
    if remove_columns:
        final_data = final_data.remove_columns(remove_columns)
    if args.save_mode=='save_to_disk':
        final_data.save_to_disk(dataset_path_after_dedup)
    elif args.save_mode=='save_to_json':
        final_data.to_json(path_or_buf=dataset_path_after_dedup)
    else:
    ...
    logger.info(f"{'Deduplicated Dataset':<{PAD}}: {dataset_path_after_dedup}")

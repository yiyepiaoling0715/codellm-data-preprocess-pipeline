...
def pattern_match(patterns, source_list):
    ...
    ##从 source_list中找patterns
    task_names = set()
    for pattern in patterns:
        for matching in fnmatch.filter(source_list, pattern):
            task_names.add(matching)
    ##?? 输入 patterns 跟  task_names 什么区别?
...
def basic_filters_per_extension(example, ext_to_filter):
    ...
    try:
        ...
        # Exclude these files
        if 'ext' not in example.keys():
            logging.error(str(e) + f":ext not in example")
        else:
...
if __name__ == "__main__":
    ...
    ##todo  改为实际的
    ##默认拉取开源code data=dataset_name   从中取subset
    dataset = load_dataset(
        args.dataset_name, split=args.split, data_dir=args.subset, use_auth_token=True,num_proc=args.num_workers,
        keep_in_memory=True,token='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',
        cache_dir='~/.cache/huggingface/datasets_filter'
    )
    logger.info(f"Dataset loaded in {time.time() - t_start:.2f} seconds")
    logger.info(f"Dataset: {dataset}")
    logger.info(f"dataset.column_names={dataset.column_names}")
    logger.info(f"dataset.dataset_size={dataset.dataset_size}")
    ##needed for domain adaptive
    ##计算 sample 数量
    if "size" not in dataset.column_names:
        logger.info("Add text size column")
        dataset = dataset.map(get_size_text)
    ##not needed for domain adaptive
    #sample中带上 license
    ...
    for filter in filters:
        if filter == "basic":
            ...
            t_start = time.time()
            ##needed for domain adaptive
            #行长度过滤  max_line_length/avg_line_length/有效alpha ratio过滤
            #size=len(dataset)
        ...
        ##??为什么全给移除了,
            ...
            ##??每个都要搞？  {language:[include, line_max, line_mean, alphanum_frac, alphabetic_frac]}
